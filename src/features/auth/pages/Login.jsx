...
export default function Login() {
      ...
      if (window.google && window.google.accounts) {
        ...
        if (btnElement) {
          google.accounts.id.renderButton(btnElement, {
            ...
            size: "large",
            shape: "pill",
            width: 320 // Must be a number (pixels)
          });
  ...
  const handleGoogleCredentialResponse = async (credentialResponse) => {
    try {
      ...
      // Store admin data for the dashboard
      if (res.data.user) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setApiError(err.response?.data?.detail || "Invalid credentials.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        .login-root { min-height: 100vh; display: grid; grid-template-columns: 1.15fr 0.85fr; font-family: 'DM Sans', sans-serif; background: #fff; }
        @media (max-width: 1024px) { .login-root { grid-template-columns: 1fr; } .visual-panel { display: none !important; } }
        .visual-panel { position: relative; overflow: hidden; background: #000; }
        ...
        .login-card { width: 100%; max-width: 448px; background: #ffffff; padding: 2.5rem; border-radius: 1.5rem; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04); border: 1px solid #e5e7eb; animation: slideInUp 0.5s cubic-bezier(0.16, 1, 0.3, 1); }
        ...
        .google-btn-container { display: flex; justify-content: center; margin-bottom: 1.5rem; min-height: 44px; }
        ...
        .input-icon { position: absolute; left: 1rem; top: 50%; transform: translateY(-50%); color: #6b7280; transition: color 0.2s; z-index: 10; }
        .input-field { width: 100%; height: 3rem; background: #ffffff; border: 1px solid #d1d5db; border-radius: 0.75rem; padding: 0 1rem 0 2.75rem; font-size: 0.95rem; color: #111827; transition: all 0.2s; outline: none; }
        .input-field:focus { border-color: #f97316; box-shadow: 0 0 0 2px #fff, 0 0 0 4px rgba(249,115,22,0.2); }
        .field-wrap:focus-within .input-icon { color: #f97316; }
        .remember-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; font-size: 0.875rem; }
        ...
        .btn-login { width: 100%; height: 3rem; background: linear-gradient(135deg, #f97316 0%, #fb923c 100%); color: #fff; border: none; border-radius: 0.75rem; font-weight: 700; font-size: 0.95rem; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 0.5rem; transition: all 0.2s; box-shadow: 0 4px 6px -1px rgba(249,115,22,0.2); }
        .btn-login:hover { transform: translateY(-2px); box-shadow: 0 10px 15px -3px rgba(249,115,22,0.3); filter: brightness(1.05); }
        .btn-login:active { transform: scale(0.98); }
        .btn-login:disabled { opacity: 0.7; cursor: not-allowed; transform: none; }
        .btn-otp { width: 100%; height: 3rem; background: #fff; border: 1px solid #d1d5db; color: #4b5563; border-radius: 0.75rem; margin-top: 1rem; font-weight: 600; font-size: 0.9rem; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 0.5rem; transition: all 0.2s; }
        ...
        .country-select { width: 100px; height: 3rem; background: #fff; border: 1px solid #d1d5db; border-radius: 0.75rem; padding: 0 0.5rem; font-size: 0.95rem; color: #111827; outline: none; cursor: pointer; }
        .country-select:focus { border-color: #f97316; box-shadow: 0 0 0 4px rgba(249,115,22,0.1); }
        .api-error { background: #fff1f2; border: 1px solid #fecaca; color: #b91c1c; padding: 0.75rem; border-radius: 0.75rem; font-size: 0.8125rem; margin-bottom: 1.25rem; display: flex; align-items: center; gap: 0.5rem; }
        .spin { animation: spin 0.8s linear infinite; }
        @keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
        .trust-row { display: flex; justify-content: center; gap: 1.5rem; margin-top: 1.5rem; font-size: 0.75rem; color: #9ca3af; }
      `}</style>
      ...
      <div className="login-root">
        <div className="visual-panel">
          ...
          <div className="panel-content">
            ...
            <div className="city-block">
              ...
              <div className="dots">
                {SLIDES.map((_, i) => (
                  <div key={i} className={`dot ${i === current ? "active" : ""}`} onClick={() => setCurrent(i)} role="button" />
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="form-panel">
          <div className="login-card">
            ...
            {regMessage && <div className="api-error" style={{ background: '#f0fdf4', color: '#166534', borderColor: '#bcf0da' }}>✅ {regMessage}</div>}
            ...
            {loginMode === "email" && (
              <form onSubmit={handleSubmit} noValidate>
                <div className="field-group">
                  <div>
                    ...
                    <div className="field-wrap">
                      ...
                      <input name="email" type="email" placeholder="e.g. name@example.com" value={form.email} onChange={handleChange} className="input-field" />
                    </div>
                    {errors.email && <p style={{ color: '#ef4444', fontSize: '0.75rem', marginTop: '0.25rem' }}>{errors.email}</p>}
                  </div>
                  <div>
                    <label className="label-text">Password</label>
                    <PasswordInput name="password" value={form.password} onChange={handleChange} placeholder="Enter your password" />
                ...
                <div className="remember-row">
                  ...
                  <Link to="/forgot-password" style={{ color: '#f97316', textDecoration: 'none', fontWeight: 600 }}>Forgot password?</Link>
                </div>

                {apiError && <div className="api-error"><ShieldCheck size={16} /> {apiError}</div>}

                <button type="submit" disabled={loading} className="btn-login">
                  {loading ? <Loader2 size={18} className="spin" /> : <>Sign In <ArrowRight size={18} /></>}
                </button>

                <button type="button" onClick={() => setLoginMode("phone")} className="btn-otp">
            ...
            {loginMode === "phone" && (
              <form onSubmit={handlePhoneSubmit} noValidate>
                <div className="field-group">
                  <div>
                    ...
                    <div className="phone-group">
                      ...
                      <input type="tel" placeholder="10-digit number" value={phone} onChange={(e) => setPhone(e.target.value)} className="input-field" style={{ paddingLeft: '1rem' }} />
                    </div>
                  </div>
                </div>
                {apiError && <div className="api-error"><ShieldCheck size={16} /> {apiError}</div>}
                ...
                <button type="button" onClick={() => setLoginMode("email")} className="btn-otp">
